    ...
    >
      ...
      <div className="absolute bottom-0 left-0 right-0 h-[2px] z-10"
        ...
        <div ref={barRef} className="h-full w-0"
          style={{ background: "linear-gradient(to right, rgba(255,255,255,.4), #fff)" }} />
      </div>
  ...
  return (
    <div
      className={`absolute inset-0 transition-opacity duration-[1100ms] ease-[cubic-bezier(.4,0,.2,1)]
        ${active ? "opacity-100 z-[2] pointer-events-auto slide-active" : "opacity-0 z-0 pointer-events-none"}`}
    ...
    >
      ...
      {/* ✅ ÚNICA MUDANÇA — paddingTop: "99px" para o conteúdo não ficar atrás da navbar */}
      <div className="relative z-[3] h-full flex items-center max-w-[1280px] mx-auto px-12 gap-12"
        ...
        <div className="flex-1 max-w-[580px]">
          ...
          <h1 className="anim-h1 font-black leading-[1.03] mb-5 text-white"
            style={{ fontSize: "clamp(44px,5.8vw,76px)", letterSpacing: "-2.5px" }}>
            {s.title[0]}<br />
            <span style={{ color: "rgba(255,255,255,.5)" }}>{s.title[1]}</span>
          </h1>

          <p className="anim-p text-[14px] leading-[1.85] mb-9 max-w-[420px]"
            style={{ color: "#555" }}>
            {s.desc}
          </p>
          ...
          <div className="anim-btns flex gap-3.5 mb-10">
            ...
            <button className="font-semibold text-[13px] px-9 py-3.5 rounded-[10px] cursor-pointer transition-all duration-200 text-white hover:-translate-y-0.5"
              style={{ background: "rgba(255,255,255,.045)", border: "1px solid rgba(255,255,255,.14)", backdropFilter: "blur(10px)" }}
